...
namespace CoreTests.Integration.Attachments
{
    public class Attachments : ApiWrapperTest
    {
        ...
        {
            return await CreateAttachment((await Given_invoice_with_no_attachments(includeOnline)), AttachmentEndpointType.Invoices, includeOnline);
        }

        private async Task<Attachment> Given_an_attachment_with_a_space_in_the_name_on_an_invoice(bool includeOnline = false)
        {
            return await CreateAttachment((await Given_invoice_with_no_attachments(includeOnline)), AttachmentEndpointType.Invoices, new FileInfo(ImageWithSpacesPath), includeOnline);
        }

        private async Task<Attachment> Given_attachment_on_credit_note(bool includeOnline = false)
        {
            return await CreateAttachment((await Given_credit_note_with_no_attachments()), AttachmentEndpointType.CreditNotes, includeOnline);
        }
