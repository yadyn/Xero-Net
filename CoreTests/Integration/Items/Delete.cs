...
namespace CoreTests.Integration.Items
{
    ...
    public class Delete : TrackedInventoryTest
    {
        ...
        {
            Assert.Throws<NotFoundException>(async () => await Api.Items.DeleteAsync(CreatedItem));
        }

        private void When_I_try_to_Delete_it_400s()
        {
            Assert.Throws<ValidationException>(async () => await Api.Items.DeleteAsync(CreatedItem));
        }

        private void Its_not_there_when_I_try_to_find_it(Item theItem)
        {
            Assert.Throws<NotFoundException>(async () => await Api.Items.FindAsync(theItem.Id));
        }
    }
}
